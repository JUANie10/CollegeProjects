...
BigInteger stringToBigInteger(char* s) {
  long tempnum = 0, counter = 0, counter1 = 0;
  ...
  if (pos == sign) {
    ...
    while(counter != strlen(s)) {
      if (first == false) {
        for (long i = 0; i <((strlen(s)-1)%POWER); i++) {
          printf("s[%ld]: %c\n", counter, s[counter]);
          if (isdigit(s[counter]) == 0) {
            printf("BigInteger Error: calling stringToBigInteger on non digits");
            exit(1);
          }
          buffer[i] = s[counter];
          counter++;
    ...
    while(counter != strlen(s)) {
      if (first == false) {
        for (long i = 0; i <((strlen(s))%POWER); i++) {
          if (isdigit(s[counter]) == 0) {
            printf("BigInteger Error: calling stringToBigInteger on non digits");
            exit(1);
          }
